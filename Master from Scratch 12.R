...
Version <- "Master from Scratch 12"
######################################################################
...
alpha.level <- 0.01#  0.01/ Besag, 0.05/ Kullforf  (NA for INLA)
...
ALLorWILMS <- "WILMS"
lambda = 0.7/100000 # 14/100000 für all neoplasms, 0.7/100000 for Wilms Tumors  (15.99/0.86 for real data )
# relative risk in risk clusters
# ClusterRR <- c(1,1.1,1.2, 1.3,1.4, 1.5, 2, 5,10,100)
ClusterRR <- c(2) # for shorter Test runs
...
Clustertype <-c ("Cluster0")#, "Cluster2")#, "Cluster6")#, "Cluster2")
...
# # to display e.g. mean exact power and relative risk 
# take out the hashtags of the next 40 lines
# Title <- "Exact Power and Relative Risk"
# IMAGE <- ggplot(filter(kombinierteListe, Clustertype %in% Clustertype), # oder Clustertype %in% c("Cluster1", "2") oder so 
#                 aes(x=`Relative.Risk` ,
#                 y=`Mean Exact Power`,
#                 color=Clustertype))+
#   geom_point()+
...
#   # geom_line()+
#   geom_smooth(se=F, level = 0.95) + # se= confidence intervall
...
#         legend.key.height = unit (30, "pt"), # color bar von oben nach unten
#         legend.key.width = unit (30, "pt"), # dicke der Color bar
#         legend.key = element_rect(colour = NA, fill = NA), # get rid of the grey background of legend
#         plot.title = element_text(hjust=0.5, size=20), # Title in der Mitte
#         plot.background=element_blank())+
#   coord_cartesian(xlim = c(0, 30))+ # sichtbaren Bereich der X achse limitieren
#   # theme(legend.position = "none") # hier ist die Legende und Colorbar weg
#   theme(legend.justification="right",legend.position="right")  # legend.position=c(1.1,0.5)) # hiermit
# 
# plot(IMAGE)
# # Save the Figure properly
# png(file =paste("Data/Figures/", Title, Sys.Date(), ".png", sep = ""), width= 10, height= 7.25,units= "in",res= 600,pointsize = 30)
